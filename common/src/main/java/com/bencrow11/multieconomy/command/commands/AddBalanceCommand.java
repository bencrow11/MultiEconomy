...
public class AddBalanceCommand implements SubCommandInterface {
	...
	public int run(CommandContext<CommandSourceStack> context) {
		...
		if (success) {
			if (amountArg == 1) {
				context.getSource().sendSystemMessage(Component.literal(Utils.formatMessage("§aSuccessfully added §b" +
		...
		context.getSource().sendSystemMessage(Component.literal(Utils.formatMessage("§cUnable to add currency to the account.",
				isPlayer)));
		return -1;
	}
